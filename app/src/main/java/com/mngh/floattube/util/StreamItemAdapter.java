...
public class StreamItemAdapter<T extends Stream> extends BaseAdapter {
    ...
    private View getCustomView(int position, View convertView, ViewGroup parent, boolean isDropdownItem) {
        if (convertView == null) {
            convertView = LayoutInflater.from(context).inflate(com.mngh.floattube.R.layout.stream_quality_item, parent, false);
        }

        final ImageView woSoundIconView = convertView.findViewById(com.mngh.floattube.R.id.wo_sound_icon);
        final TextView formatNameView = convertView.findViewById(com.mngh.floattube.R.id.stream_format_name);
        final TextView qualityView = convertView.findViewById(com.mngh.floattube.R.id.stream_quality);
        final TextView sizeView = convertView.findViewById(com.mngh.floattube.R.id.stream_size);
